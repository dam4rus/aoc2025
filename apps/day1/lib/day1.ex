...
R24"
  ...
  """
  @spec rotate(number(), direction(), number()) :: number()
  def rotate(dial, :left, rotation) when is_number(dial) and is_number(rotation) do
    case dial - rem(rotation, 100) do
      dial when dial < 0 -> 100 - (rem(dial, 100) |> abs)
      dial -> dial
    end
  end

  def rotate(dial, :right, rotation) when is_number(dial) and is_number(rotation),
    do: (dial + rem(rotation, 100)) |> rem(100)

  @doc """
  Processes input lines.

  Returns the number the dial rest at 0.
  """
  @spec process_input([String.t()]) :: number()
  def process_input(lines) when is_list(lines) do
    process_input(50, lines, 0)
  end

  @doc """
  Processes the embedded real input lines.

  Return the number the dial rest at 0.
  """
  @spec process_input() :: number()
  def process_input() do
    process_input(@input |> String.split("\n"))
  end

  @spec process_input(number(), [String.t()], number()) :: number()
  defp process_input(dial, [line], zero_count)
  ...
  defp process_input(dial, [line | tail], zero_count)
       when is_number(dial) and is_binary(line) and is_number(zero_count) do
    {:ok, rotation_direction} = String.at(line, 0) |> rotation_direction
    ...
    case rotate(
      ...
      dial when dial == 0 ->
        process_input(dial, tail, zero_count + 1)

      dial ->
        process_input(dial, tail, zero_count)
    end
  end
